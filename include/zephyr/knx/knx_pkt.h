...
typedef enum knx_pkt_type {
	KNX_PKT_DATA_INDIVIDUAL,
	KNX_PKT_DATA_GROUP,
	KNX_PKT_DATA_BROADCAST,
	KNX_PKT_DATA_SYSTEM_BROADCAST,
} knx_pkt_type_t;
...
struct knx_pkt {
	...
	/* Filled by Ph_Data__ind */
	uint8_t buf[MAX_KNX_TELEGRAM_SIZE];
	uint16_t buf_len; /* up to 263 for extended frames — see KNX spec 3/2/2 */
